...
1. `codex-controller.ts` 用 PowerShell 查找 `OpenAI.Codex` Store 包。
...
7. `injector.ts` 只接受同一 browser ID、回环 WebSocket、`app://` page target。
...
`media-server.ts` 主要服务 Studio 预览和视频 Range，不应重新拿来作为 Codex 背景源。
...
1. `src/shared/contracts.ts`
   - `DisplaySettings`
   - `DEFAULT_SETTINGS`
2. `src/main/settings.ts`
   - `normalizeSettings()` 范围和 fallback
...
- 先 `npm run dev` 调试 Studio。
- 只看 Studio UI 时用 `npm run dev:web`。
- 用一次性 Node `.mjs` 连接 Codex CDP。
- 把 DOM、计算样式和截图作为证据。
